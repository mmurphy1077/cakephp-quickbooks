			...
			<table id="rate_select_container" class="standard tight nohover clear sortable">
				...
				if(!empty($this->data)) :
					foreach($this->data as $data) : 
						echo $this->element('business360'.DS.'license_table_row', array('index' => $i, 'data' =>$data['License']));
						$i = $i + 1;
					endforeach; 
				endif; 
				...
				if($i <= 4) {
					for($i; $i <= 4; $i++) {
						echo $this->element('business360'.DS.'license_table_row', array('index' => $i, 'data' => null, 'type' => null));
					}
				} else {
					echo $this->element('business360'.DS.'license_table_row', array('index' => $i, 'data' =>null, 'type' => null));
				} ?>
