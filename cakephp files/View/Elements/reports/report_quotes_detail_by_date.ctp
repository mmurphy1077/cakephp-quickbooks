...
$html = $html . '<span class="no_data">' . __('No data exists for the parameters selected.') . '</span>';
		...
		foreach($result['data'] as $data) :
			...
			if (date('m/d/Y', strtotime($data['Quote']['created'])) != $current_sort_header) {
				$insert_table_close = true;
				if($current_sort_header == 'start') {
					$insert_table_close = false;
				}				
				
				$current_sort_header = date('m/d/Y', strtotime($data['Quote']['created']));
...
$html = $html . '				<div class="label inline">By Date:</div>';
$html = $html . '				<b>' . $current_sort_header . '</b>';
$html = $html . '			</div>';
$html = $html . '			<div class="report_table_container">';
$html = $html . '				<table>';
$html = $html . '					<tr>';
