...
$html = $html . '<span class="no_data">' . __('No data exists for the parameters selected.') . '</span>';
		...
		foreach($result['data'] as $data) :
			...
			if ($data['Quote']['account_rep_id'] != $current_sort_header) {
				$insert_table_close = true;
				if($current_sort_header == 'start') {
					$insert_table_close = false;
				}					
				$current_sort_header = $data['Quote']['account_rep_id'];
				if(empty($current_sort_header)) {
					$current_sort_header_dislpay = __('Unassigned');
				} else {
					$current_sort_header_dislpay = $data['AccountRep']['account_rep_name_last'] . ', ' . $data['AccountRep']['account_rep_name_first'];
				}
...
$html = $html . '				<div class="inline">By Rep:</div>';
...
$html = $html . '						<th class="rowrap">Amount</th>'; 
$html = $html . '						<th class="rowrap">Status (Current)</th>';
$html = $html . '					</tr>';
			endif; 
...
$html = $html . '			 	<td>' . $result['status'][$data['Quote']['status']] . '</td>';
$html = $html . '			</tr>';
			if($display_details): 
