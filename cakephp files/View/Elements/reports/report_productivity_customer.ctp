<?php
$title = "Customer Productivity Summary Report";
if($display_details) {
	$title = 'Customer Productivity Detail Report';
}
?>
<div id="application_report_container">
	...
	<?php 	foreach($data['Data'] as $key=>$customer) : ?>
		<div class="productivity_report_table_container">
			<table>
				<tr>
					<th class="nowrap" colspan="3"><?php echo $customer['Customer']['name']; ?></th>
					<?php 
					$company_profit_loss = 0;
					if(array_key_exists('InvoiceStats', $customer['Stats']) && array_key_exists('score_productivity', $customer['Stats']['InvoiceStats'])) {
						$company_profit_loss = $customer['Stats']['InvoiceStats']['score_productivity'];
					}
					?>
					<th class="align-right nowrap" colspan="3">Combined Profit/Loss: <?php echo $company_profit_loss; ?>% </th>
				</tr>
				<?php 
				...
				<tr class="customer-productivity-row">
					...
					?>
					<td><?php echo $this->Web->address($address, false); ?></td>
					<td><?php #debug($order); ?>
