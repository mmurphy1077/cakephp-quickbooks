...
?>
<?php 
echo $this->element('js'.DS.'jquery', array('ui' => 'cluetip')); 
echo $this->element('js'.DS.'jquery', array('ui' => 'datepicker'));
echo $this->Html->script('creationsite/datepicker.init', false);
echo $this->Html->script('creationsite/call.log', array('inline' => false));
echo $this->Html->script('creationsite/ajax', false);
?>
<?php $this->start('leftbar'); ?>
		<div class="widget left bg">
			<h3><?php echo __('Statistics eeeee'); ?></h3>
			<?php echo $this->Form->create(); ?>
			<?php #echo $this->Form->input('view_by', array('type' => 'radio', 'options' => $optionsStats)); ?>
			<div class="grid flat">
				<div class="col-2of5">
					<?php echo $this->Form->input('Start', array('div' => 'input text short', 'id' => 'datepicker_from')); ?>
				</div>
				<div class="col-2of5">
					<?php echo $this->Form->input('End', array('div' => 'input text short', 'id' => 'datepicker_to')); ?>
				</div>
				<div class="col-1of5">
...
<?php $this->end(); ?>
				<div class="widget center">
					<?php echo $this->Html->link(__('Show More'), array('controller' => 'contacts', 'action' => 'index'), array('class' => 'button medium blue right')); ?>
					...
					<table class="standard">
						...
						<?php 	foreach($results as $key=>$result) : 
							...
							<tr id="lead_row_<?php echo $result['Contact']['id']; ?>" title="<?php echo $result['Contact']['name_forward']; ?>" rel="#lead<?php echo $key; ?>">
								<td class="nowrap">
								...
								<td class="center">
									<?php echo $this->Form->hidden('notes_public', array('id'=>'note_public_'.$result['Contact']['id'], 'value'=>nl2br($result['Contact']['notes']))); ?>
									<a class="div-clicktip-short transfer_id" id="<?php echo $result['Contact']['id']; ?>" rel="#log-call" href="#log-call" title="<?php echo htmlentities($result['Contact']['name_last']); ?> &mdash; <?php echo __('Log Phone Call'); ?>">
										<?php echo $this->Html->image('icon-bubble-16.png'); ?>
									</a>
								</td>
								<td class="nowrap">
								...
								<td><?php echo $result['Address']['city']; ?>,&nbsp;<?php echo $result['Address']['st_prov']; ?></td>
								<?php 
									if(empty($result['Contact']['account_rep_id'])) :  ?>
								<td class="account_rep">
									<?php echo $this->Form->input('account_rep_id', array('empty' => true, 'label'=>false, 'id'=>'assign_lead_to_'.$result['Contact']['id'], 'class'=>'index_assign_to', 'div' => 'input select medium')); ?>
								</td>
								<td class="actions_small">
									<?php echo $this->Html->link(__('Save'), '#', array('id'=>$result['Contact']['id'], 'class'=>'assign_lead_to')); ?>
									<?php echo $this->Html->image('loader.gif', array('id'=>'ajax_loader_'.$result['Contact']['id'], 'class'=>'ajax_loader', 'div'=>false));?>
									<?php echo $this->Html->image('icon-success2.png', array('id'=>'ajax_loader_success_'.$result['Contact']['id'], 'class'=>'ajax_loader_success', 'div'=>false));?>
									<?php echo $this->Html->image('icon-error4.png', array('id'=>'ajax_loader_error_'.$result['Contact']['id'], 'class'=>'ajax_loader_error', 'div'=>false));?>
								</td>
								<?php 
									else: ?>
								<td class="account_rep" colspan="2">
									<?php echo $this->Web->humanName($result['AccountRep']); ?>
								</td>
								<?php endif; ?>	
							</tr>
						<?php 	endforeach;	?>
				...
				<div class="widget center">
					...
					<table class="standard">
						...
						<?php 	foreach($my_quotes as $key1=>$my_quote) : 
									...
									<tr title="<?php echo $my_quote['Quote']['name']; ?>" rel="#qt<?php echo $key1; ?>">
				...
				<div class="widget center">
					...
					<table class="standard">
						...
						<?php 	foreach($all_quotes as $key1=>$all_quote) : 
									...
									<tr title="<?php echo $all_quote['Quote']['name']; ?>" rel="#qt<?php echo $key1; ?>">
				...
				<div class="widget center">
					...
					<table class="standard">
						...
						<?php 	foreach($quotes_approved as $key1=>$approved_quote) : 
									...
									<tr title="<?php echo $approved_quote['Quote']['name']; ?>" rel="#qt<?php echo $key1; ?>">
