...
class UploadableBehavior extends ModelBehavior {
	...
	function beforeSave(Model $model) {
		...
		if (!empty($model->data[$model->alias][$settings['validationKey']])) {
			...
			if(empty($aws)) {
				 ...
				 */
				$cloud_status_id = 0;
				if ($file = $this->__move($model->data[$model->alias], $uploadPath.DS, $settings['renameFile'])) {
				 ...
				 * 		CLOUD STORAGE
			 	 */
				$cloud_status_id = 1;
				if ($file = $this->__move_cloud($model->data[$model->alias], $model->alias.DS, $settings['renameFile'])) {
					$successful_upload = true;
				}
			}
			if ($successful_upload) {	
				...
				$model->data[$model->alias]['creator_id'] = $model->data[$model->alias]['creator_id'];
				$model->data[$model->alias]['model'] = $model->data[$model->alias]['model'];
				$model->data[$model->alias]['foreign_key'] = $model->data[$model->alias]['foreign_key'];
				$model->data[$model->alias]['title'] = $model->data[$model->alias]['title'];
				$model->data[$model->alias]['mime_type'] = $model->data[$model->alias]['type'];
				$model->data[$model->alias]['bytes'] = $model->data[$model->alias]['size'];
				$model->data[$model->alias][$settings['validationKey']] = $file['name'].'.'.$file['extension'];
			} else {
	...
	function __move_cloud($source, $destination, $rename = true) {
		...
		if(!empty($subfolder)) {
			$destination_path = Configure::read('AWS.'.Configure::read('Environment.platform').'.sub-folder').DS.$destination_path;
		}
