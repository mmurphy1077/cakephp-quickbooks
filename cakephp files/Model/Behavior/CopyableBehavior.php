 ...
 */
	...
	public $business360_settings = array();
 ...
 * @return boolean
 */
	public function copy($Model, $id, $skipFields=null) {
		if(!empty($skipFields)) {
			foreach($skipFields as $key=>$data) {
				$this->business360_settings[$key]['stripFields'] = $data;
			}
		}
		
		// Set flag so models can override specific copy behavior in callbacks
		$Model->copying = true;
 ...
 */
	private function __copyRecord($Model) {
		...
		return $Model->saveAll($this->record, array('validate' => false, 'deep' => true));
	}
 ...
 */
	private function __stripFields($Model, $record) {
		if(array_key_exists($Model->alias, $this->business360_settings)) {
			$strip_fields = $this->business360_settings[$Model->alias]['stripFields'];
		} else {
			$strip_fields = $this->settings[$Model->alias]['stripFields'];
		}
