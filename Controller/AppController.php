...
#		$this->Auth->logoutRedirect = array('controller' => 'users', 'action' => 'login');
	...
	public function beforeRender() {
		...
		$this->set('__states', $State->getList());
		$Quote = ClassRegistry::init('Quote');
		$this->set('statuses_quote', $Quote->statuses);
		$this->set('statuses_order', $Quote->Order->statuses);
		$this->set('__locations', $Quote->Order->Location->getActiveList());
		$this->set('statuses_approval', $Quote->QuoteTask->approval_statuses);
		$this->set('statuses_task', $Quote->QuoteTask->task_statuses);
		$this->__setCompanyEnvironment();
		$Message = ClassRegistry::init('Message');
		if(!empty($this->user)) {
			$this->set('__user_unread_message_count', $Message->getUserUnreadMessageCount($this->user));
			$this->set('__active_timers', $this->__getTimers($this->user));
		}
